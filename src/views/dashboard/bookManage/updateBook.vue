<template>
  <div class="app-container">
    <el-form ref="form" :model="form" label-width="150px" :rules="rules">
      ...
      <el-form-item label="纸质图书图片上传" prop="bookPicture">
        <el-upload
          name="uploadFile"
          class="avatar-uploader"
          action="http://localhost:8888/bookManagement/appleBook/uploadBookImg"
...
export default {
  ...
  created () {
    const _this = this
    getReaderById(this.$route.query.bookId).then(function (response) {
      console.log('%c [ response ]', 'font-size:13px; background:pink; color:#bf2c9f;', response)
      _this.form.bookId = response.appleBook.bookId
      _this.form.bookAuthor = response.appleBook.bookAuthor
      _this.form.bookName = response.appleBook.bookName
      _this.form.bookRepertory = response.appleBook.bookRepertory
      _this.form.bookPicture = response.appleBook.bookPicture
      _this.form.bookIsbn = response.appleBook.bookIsbn
      _this.form.bookIntro = response.appleBook.bookIntro
      _this.form.bookPicture = response.appleBook.bookPicture
        ...
        if (valid) {
          updateReader(this.form).then(response => {
            if (response.result != 0) {
              console.log(response.appleBook.bookName + '======')
              this.$message(
                '名称为：' + response.appleBook.bookName + '的纸质图书修改成功'
              )
            } else {
              this.$message('添加失败')
            }

<template>
  <el-card class="box-card" shadow="never">
    <div slot="header" class="clearfix">
      <span>纸质图书名：{{ this.form.eBookName }}</span>
    ...
    <div class="text item">
      <span>图书作者：{{ this.form.eBookAuthor }}</span><br /><br /><br />
      <span>图书ISBN码：{{ this.form.eBookIsbn }}</span><br /><br /><br />
      <span>图书出版社：{{ this.form.eBookPress }}</span><br /><br /><br />
      <span>图书分类名：{{ this.form.categoryName }}</span><br /><br /><br />
      <span>图书简介：{{ this.form.eBookIntro }}</span><br /><br /><br />
      ...
      style="float: right; position: absolute; right: 5%; bottom: 2% " 
      type="primary" 
      @click.native="openEBookFile(this.form.eBookFileUrl)">
        阅读该电子书
        <i class="el-icon-view el-icon--right"></i> 
      </el-button> -->
    </div>
...
export default {
  ...
  created() {
    ...
    getEBookById(this.$route.query.ebookId).then(function (response) {
      ...
      _this.ebookList = response.eBook
    })
  },
  methods: {
    fetchData() {},
